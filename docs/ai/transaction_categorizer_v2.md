# Transaction Categorizer – v2 Prompt Spec

Goal: Given a single transaction JSON plus some context, return a structured classification object used to populate the `transactions` table, including a numeric confidence score for how accurate the chosen category is.
...
- `confidence`: integer from `1–100` representing how confident the model is that the selected `category_name` and related flags are correct (higher is more confident).
...
9. **Manual Overrides**
   ...
   - When an override applies and the pattern is very specific, confidence should typically be high (for example, 90 or above). When the model is inferring from weak or ambiguous signals, use a lower confidence score.
